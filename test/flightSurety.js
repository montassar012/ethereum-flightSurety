...
contract('Flight Surety Tests', async (accounts) => {
  ...
  describe('Operations and Settings', () => {
    it(`Has correct initial isOperational() value`, async () => {
        console.log(`************ status $***********`);
        let statusContract = await app.isOperational();
      console.log(`************ ${statusContract} , ${await app.isOperational({ from: config.contractOwner })}$***********`);
      assert.equal(statusContract ,true, 'Incorrect initial operating status value');
    });

    it(`Can block access to setOperationalStatus() for non-Contract Owner account`, async () => {
    ...
    it(`Can allow access to setOperationalStatus() for Contract Owner account`, async () => {
      let accessDenied = false;
      try {
  ...
  describe('Business Logic', () => {
    ...
    describe('Scenario: Only existing airline may register a new airline until there are at least four airlines registered', async () => {
      ...
      before('Nominate airlines', async () => {
        let tx2 = await app.nominateAirline(actors.airline2,"airline2", { from: actors.airline1 });
        expectEvent(tx2, 'AirlineNominated', {
          airlineAddress: actors.airline2,
        });

        let tx3 = await app.nominateAirline(actors.airline3,"airline3", { from: actors.airline1 });
        expectEvent(tx3, 'AirlineNominated', {
          airlineAddress: actors.airline3,
        });

        let tx4 = await app.nominateAirline(actors.airline4,"airline4", { from: actors.airline1 });
        expectEvent(tx4, 'AirlineNominated', {
          airlineAddress: actors.airline4,
        });

        let tx5 = await app.nominateAirline(actors.airline5,"airline5", { from: actors.airline1 });
