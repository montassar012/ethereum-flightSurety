...
const registerFlights = async () => {
  ...
  try {
      ...
      for (let i = 0; i < airlineCount; i++) {
          ...
          for (let k = 0; k < 2; k++) {
              const callSign = `${airline.companyName.toUpperCase()}${i}0${k}`;
              const timestamp = Date.now() + Math.floor(Math.random() * 10000000);
              const flight = {airline: airlineAddress,callSign, timestamp};
              console.log({flight});
              // setFlights(flights => flights.concat({flight}));
              await flightSuretyApp.methods.registerFlight(airline.airlineAddress,callSign, timestamp).send({
